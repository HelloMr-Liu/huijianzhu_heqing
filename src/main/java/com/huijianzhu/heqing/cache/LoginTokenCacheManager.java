...
public class LoginTokenCacheManager {
     ...
     * @param userIds
     */
    public void deleteUserOnLoginContent(String[] userIds) {
        //创建一个集合存储当前用户id对应在缓存中的登录标识
        List<String> loginTokenByUserIdList = new ArrayList<>();

        //遍历已删除用户对应的id信息集
        Arrays.asList(userIds).forEach(
                uid -> {
                    loginTokenCahce.entrySet().stream().forEach(
                            e -> {
                                if (e.getValue().getUserId().toString().equals(uid)) {
                                    //用户登录缓存中有对应的删除用户id
                                    loginTokenByUserIdList.add(e.getKey());
                                }
                            }
        ...
        //将这个删除用户对应之前的登陆表示信息删除
        loginTokenByUserIdList.forEach(
                e -> {
                    loginTokenCahce.remove(e);
                }
        );
        log.info("删除了对应的登录标识信息：" + loginTokenByUserIdList.size());
    }
}
